...
var confirmButtons = []command.Button{
	{Label: "Yes", Style: discord.ButtonStyleDanger, Value: "yes"},
	{Label: "No", Style: discord.ButtonStyleSecondary, Value: "no"},
}

func init() {
	userOpt := func(description string) command.Option {
	...
	command.Register(
		...
		&command.Command{
			...
			Options:     []command.Option{userOpt("The member to unban")},
			GuildOnly:   true,
			Permissions: discord.PermissionManageRoles,
			Handler:     NSFWUnban,
		},
...
func SetNSFWRole(c *command.Ctx) {
	role := c.Role("role")
	answer, ok := c.Ask(discord.MessageCreate{
...
func RequestNSFW(c *command.Ctx) {
	...
	if guild, ok := c.Client.Caches.Guild(c.GuildID); ok {
		guildName = guild.Name
	}

	dm, err := c.Client.Rest.CreateDMChannel(c.Author.ID)
	if err != nil {
	...
	sendDM := func(content string) {
		c.Client.Rest.CreateMessage(dm.ID(), discord.MessageCreate{Content: content})
	}
	answer, ok := command.AskInChannel(c.Client, dm.ID(), discord.MessageCreate{Content: terms}, c.Author.ID, []command.Button{
		{Label: "I agree", Style: discord.ButtonStyleSuccess, Value: "agree"},
		{Label: "Cancel", Style: discord.ButtonStyleSecondary, Value: "cancel"},
	}, optInTimeout)
	if !ok {
		sendDM("This request has been aborted automatically")
		return
	}
	if answer != "agree" {
		sendDM("Request cancelled.")
		return
	}

	if err := c.Client.Rest.AddMemberRole(c.GuildID, c.Author.ID, roleID); err != nil {
		log.Default().Println("Error adding NSFW role: " + err.Error())
		sendDM("I couldn't give you the role. Ask a moderator to check my permissions!")
		return
	}
	sendDM("Done.")
}
...
func UnNSFW(c *command.Ctx) {
	...
	if err := c.Client.Rest.RemoveMemberRole(c.GuildID, c.Author.ID, roleID); err != nil {
...
func NSFWBan(c *command.Ctx) {
	...
	if roleID := nsfwRole(c.GuildID); roleID != 0 {
		if err := c.Client.Rest.RemoveMemberRole(c.GuildID, target.ID, roleID); err != nil {
			log.Default().Println("Error removing NSFW role: " + err.Error())
		}
	}
...
func nsfwRole(guildID snowflake.ID) snowflake.ID {
...
func banReason(guildID, userID snowflake.ID) (string, bool) {
	var reason string
